...
interface QueueCardProps {
  ...
  onMoveToStart: () => void;
  onMoveToEnd: () => void;
  onShowInfo: (item: QueueItem) => void;
}
...
export function QueueCard({
  ...
  return (
    <ContextMenu.Root>
      ...
      <ContextMenu.Portal>
        <ContextMenu.Content className="z-50 min-w-30 border border-border bg-popover text-popover-foreground">
          <ContextMenu.Item
            onSelect={() => onShowInfo(item)}
            className="cursor-default px-2 py-1 text-sm data-[disabled]:pointer-events-none data-[disabled]:opacity-50 data-[highlighted]:bg-accent"
          >
            Ver información
          </ContextMenu.Item>
